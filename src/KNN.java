...
public class KNN {
	...
	static String assignLabel(projectedTrainingMatrix[] trainingSet, Matrix testFace, int K, Metric metric){
		return findKNN(trainingSet,testFace,K,metric);
	}
	
	//testFace has been projected to the subspace
	static String findKNN(projectedTrainingMatrix[] trainingSet, Matrix testFace, int K, Metric metric){
		...
		for(i = 0; i < K; i++){
			...
			System.out.println("index: "+i+" distance: "+trainingSet[i].distance);
